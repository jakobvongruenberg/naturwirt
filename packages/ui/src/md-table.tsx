...
export const MDTable = <T,>(params: {
  ...
  const {
    ...
    data,
    columns,
    tableName = t('UI.Table.TableDefaultName'),
    rowsName = t('UI.Table.RowsDefaultName'),
    downloadCSV,
  } = params
    ...
    <>
      <div className='mt-4 flex flex-row items-center justify-between py-4'>
        <div className='flex flex-1 items-center gap-4'>
          <Input
            placeholder={`${t('UI.Table.Search')} ${rowsName}...`}
            value={searchTerm}
            onChange={(event) => {
          ...
          />
          <div>
            {t('UI.Table.Showing')} {table.getRowModel().rows?.length ?? 0} /{' '}
